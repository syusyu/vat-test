...
module.exports.prepareSetting = async (testCase) => {
...
module.exports.fetchOrderHead = async (orderNo) => {
    const orderHeads = await connectDB(backDBConnStr, async (resolve, reject, conn) => {
        conn.query(orderHeadSql(orderNo), (err, data) => err ? reject(err) : resolve(data));
    });
